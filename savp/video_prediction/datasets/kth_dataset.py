...
def save_tf_record(output_fname, sequences, ids=None):
    ...
    with tf.python_io.TFRecordWriter(output_fname) as writer:
        for sequence in sequences:
            ...
            features = tf.train.Features(feature={
                ...
                'shape_ids': _floats_list_feature(ids),
            })
...
def read_frames_and_save_tf_records(output_dir, video_dirs, image_size, sequences_per_file=128):
    ...
    for video_iter, video_dir in enumerate(video_dirs):
        ...
        for frame_fnames_iter, frame_fnames in enumerate(d['files']):
            ...
            ids = None
            action = video_dir.split('/person')[0].split('/')[-1]
            action_id = ACTION_TO_ID_MAP[action]
            ids = [action_id] * len(frames)
            ...
            if (len(sequences) == sequences_per_file or
                ...
                save_tf_record(output_fname, sequences, ids=ids)
                sequences[:] = []
