...
class SAVPCell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, inputs, mode, hparams, reuse=None):
        ...
        if 'shape_ids' in inputs and self.hparams.use_shape:
            output_size['shape_embeddings'] = tf.TensorShape([5])
    ...
    def call(self, inputs, states):
        ...
        if 'shape_ids' in inputs and self.hparams.use_shape:
            print("Use shape!")
            # Todo: change hardcoded 44 shapes!
            if 'shape_embeddings' not in inputs:
                shape_ids = tf.reshape(tf.cast(inputs['shape_ids'], tf.uint8),
                                       [-1])
                id_encoded = dense(tf.one_hot(shape_ids, 44), 5)
            else:
                id_encoded = inputs['shape_embeddings']
            state_action.append(id_encoded)
            state_action_z.append(id_encoded)
        ...
        if 'shape_ids' in inputs and self.hparams.use_shape:
            outputs['shape_embeddings'] = id_encoded
        if self.hparams.transformation == 'flow':
...
def generator_given_z_fn(inputs, mode, hparams):
    ...
    if 'push_name' in inputs:
        inputs.pop('push_name')
