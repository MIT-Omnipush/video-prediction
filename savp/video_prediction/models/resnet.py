...
class ShapeEmbeddingModel(object):
    ...
    def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
                 ...
                 dtype=DEFAULT_DTYPE, mode='train',
    ...
    def build_graph(self, inputs):
        ...
        shape_ids = tf.cast(inputs['shape_ids'][:, 0], tf.int32)
        batch_size = shape_ids.get_shape()[0]
        shape_id_to_embeddings_map = tf.tile(
          tf.expand_dims(inputs['shape_id_to_embeddings_map'], 0), [batch_size, 1, 1])
        shape_embeddings = tf.squeeze(tf.batch_gather(shape_id_to_embeddings_map, shape_ids))
        self.shape_ids = shape_ids
        self.shape_embeddings = shape_embeddings

        inputs = inputs['images'][:, 0]
        ...
        self.loss = tf.losses.mean_squared_error(shape_embeddings, self.logits)

        optimizer = tf.train.AdamOptimizer(self.lr, self.beta1, self.beta2)
        with tf.name_scope('compute_gradients'):
            gradvars = optimizer.compute_gradients(self.loss, var_list=tf.trainable_variables())
        with tf.name_scope('apply_gradients'):
            self.train_op = optimizer.apply_gradients(gradvars)

        self.saveable_variables = [var for var in tf.global_variables() if var not in original_global_variables]
        return inputs
