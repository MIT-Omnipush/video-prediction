...
def main():
    ...
    sample_ind = 0
    shape_id_to_embeddings_map = dict()
    while True:
        ...
        fetches = {'shape_ids': model.inputs['shape_ids']}
        fetches.update(model.outputs.items())
        results = sess.run(fetches, feed_dict=feed_dict)
        # print(results['shape_ids'], results['shape_embeddings'])
        for i in range(args.batch_size):
            shape_id = results['shape_ids'][i][0][0]
            shape_embedding = results['shape_embeddings'][i][0]
            shape_id_to_embeddings_map[shape_id] = shape_embedding
        if len(shape_id_to_embeddings_map.keys()) == 23:
            np.save('./metadata/shape_id_to_embeddings_map.npy', shape_id_to_embeddings_map)
