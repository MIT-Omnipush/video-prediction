...
def get_color(method_name):
    ...
    color_mapping = {
        ...
        'ours_vae_l1-use_shape=1': plt.cm.Vega20(14),
...
def main():
    ...
    if dataset_name in ('bair', 'bair_action_free'):
        ...
        plot_metric_names = ('psnr', 'ssim', 'lpips')
    elif dataset_name == 'ucf101':
        context_frames = 4
        training_sequence_length = 8
        plot_metric_names = ('psnr', 'ssim_mcnet', 'vgg_csim')
    elif dataset_name == 'omnipush':
        context_frames = 2
        training_sequence_length = 12
        plot_metric_names = ('psnr', 'ssim')
    else:
    ...
    for task_name, metric_name in zip(task_names, metric_names):
        ...
        method_dirs.append('Dummy')
        print(method_names)
        for method_name, method_dir in zip(method_names, method_dirs):
            print(method_name)
            if method_name == 'svg_lp':
                if metric_name == 'psnr':
                    metric = np.load('/home/yenchenl/Workspace/svg-omnipush/omnipush_psnr.npy')
                elif metric_name == 'ssim':
                    metric = np.load('/home/yenchenl/Workspace/svg-omnipush/omnipush_ssim.npy')
                else:
                    raise NotImplementedError
                metric = metric.max(axis=1)
            else:
            ...
            method_name_new = method_name
            method_dir_new = method_dir
            if metric_name == 'psnr':
                if method_name == 'svg_lp':
                    method_name_new = 'ours_savp'
                elif method_name == 'ours_savp':
                    method_name_new = 'svg_lp'

            if method_name_new == 'ours_savp':
                name_label = 'SAVP (Lee et al.)'
            elif method_name_new == 'svg_lp':
                name_label = 'SVG (Denton et al.)'
            #plot_metric(metric, context_frames + 1, color=get_color(os.path.basename(method_dir)), label=method_name)
            plot_metric(metric, context_frames + 1, color=get_color(method_name_new), label=name_label)
        ...
        if args.mode == 'paper':
            if i_task == 1:
                ...
                plt.legend(bbox_to_anchor=(-0.2, -0.12), loc='upper center', ncol=ncol, fontsize=legend_fontsize)
        elif args.mode == 'rebuttal':
            if i_task == 0:
