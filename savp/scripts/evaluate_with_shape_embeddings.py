...
def main():
    ...
    input_phs['shape_embeddings'] = tf.placeholder(tf.float32, [args.batch_size, dataset.hparams.sequence_length, 5], '%s_ph' % 'shape_embeddings')
    ...
    shape_embeddings = np.load('./metadata/shape_ids_and_embeddings.npy').item()['logits']
    ...
    while True:
        ...
        try:
            ...
            se = shape_embeddings[sample_ind:sample_ind+args.batch_size, :]
            se = np.tile(se[:, np.newaxis, :], [1, 30, 1])
            input_results['shape_embeddings'] = se
