...
for i in range(0, opt.N, opt.batch_size):
    print("Evaluating examples %d to %d" % (i, (i+opt.batch_size)))
    # plot train
    if opt.plot_train:
    ...
    print(names)
    ssim, psnr = evaluate(test_x, i, names)
    all_ssim.append(ssim)
    all_psnr.append(psnr)

ssim = np.concatenate(all_ssim, axis=0)
psnr = np.concatenate(all_psnr, axis=0)

np.save('./omnipush_ssim.npy', ssim)
np.save('./omnipush_psnr.npy', psnr)
